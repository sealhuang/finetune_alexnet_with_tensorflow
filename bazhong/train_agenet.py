...
def model_train(train_imgs, train_ages, train_labels,
    ...
    filewriter_path = os.path.join(current_dir, 'log','agenet_cls_tensorboard')
    checkpoint_path = os.path.join(current_dir, 'log','agenet_cls_checkpoints')
    ...
    # Initialize model
    model = FCNet(age, keep_prob, num_classes, is_train)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(num_epochs):
            ...
            for step in range(train_batches_per_epoch):
                ...
                sess.run(train_op, feed_dict={age: age_batch,
                                              y: label_batch,
                                              is_train: True,
                                              lr: current_lr,
                ...
                if step % display_step == 0:
                    s = sess.run(merged_summary, feed_dict={age: age_batch,
                                                            y: label_batch,
                                                            is_train: False,
                                                            lr: current_lr,
                                                            keep_prob: 1.0})
            ...
            for _ in range(train_batches_per_epoch):
                img_batch, age_batch, label_batch = sess.run(next_batch)
                acc = sess.run(accuracy, feed_dict={age: age_batch,
                                                    y: label_batch,
                                                    is_train: False,
                                                    lr: current_lr,
                                                    keep_prob: 1.0})
            ...
            for _ in range(val_batches_per_epoch):
                img_batch, age_batch, label_batch = sess.run(next_batch)
                acc, pl, tl = sess.run([accuracy, pred_label, true_label], feed_dict={age:age_batch, y:label_batch, is_train:False, lr:current_lr, keep_prob: 1.0})
                test_acc += acc
                test_count += 1
