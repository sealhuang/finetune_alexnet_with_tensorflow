...
class ImageDataGenerator(object):
    ...
    """
    def __init__(self, img_list, age_list, label_list, mode, batch_size,
        ...
        self.ages = tf.convert_to_tensor(self.age_list, dtype=tf.float32)
        self.labels = tf.convert_to_tensor(self.label_list, dtype=tf.int32)

        # create dataset
        data = Dataset.from_tensor_slices((self.imgs, self.ages, self.labels))
    ...
    def _shuffle_lists(self):
        ...
        for i in np.random.permutation(self.data_size):
            ...
            self.age_list.append(ages[i])
            self.label_list.append(labels[i])

    def _parse_function_train(self, filename, age, label):
        ...
        return img_gray, age, one_hot

    def _parse_function_inference(self, filename, age, label):
